  ...
  async function handleSaveDocumentAnalysis(conversation: ConversationItem) {
    ...
    try {
      ...
      const suggestedReply = String(
        data?.studentReplySuggestion ||
          data?.normalizedAnalysis?.studentReplySuggestion ||
          ""
      ).trim();
      ...
      if (suggestedReply) {
        setReplyContentById((current) => ({
          ...current,
          [conversation.id]: suggestedReply,
        }));
      }
      ...
      setSuccessById((current) => ({
        ...current,
        [conversation.id]: suggestedReply
          ? "Análise salva na memória técnica. Revise a resposta sugerida abaixo e clique em Responder para enviá-la ao aluno."
          : data?.warning || data?.message || "Análise salva. Escreva uma resposta ao aluno antes de concluir.",
      }));
