...
export default async function DashboardPage() {
  ...
  const userName = sessionUser?.name || 'Usuário';
  const labels = {
    ...
    unansweredQuestionsCard: isGestor
      ? 'Dúvidas sem resposta de todos os alunos'
      : 'Dúvidas sem resposta dos meus alunos',
    ...
    managementNoticesCard: isGestor
      ? 'Avisos da gestão de todos os alunos e professores'
      : 'Avisos da gestão direcionado a mim',
    managementMessagesCard: isGestor
      ? 'Mensagens da gestão todos os alunos e professores'
      : 'Mensagens da gestão direcionado a mim',
    ...
    managementNoticesList: isGestor
      ? 'Avisos da gestão de todos os alunos e professores'
      : 'Avisos da gestão direcionado para mim',
    managementMessagesList: isGestor
      ? 'Mensagens da gestão todos os alunos e professores'
      : 'Mensagens da gestão direcionado para mim',
...
const pendingNotices = notices.filter((notice) => {
  ...
  if (isTeacher) {
    /*
     * Regra:
     * aviso enviado para aluno específico não aparece para professor,
     * mesmo que o aluno esteja vinculado a ele.
     *
     * Professor só vê, nesse bloco, avisos gerais para todos os alunos,
     * para acompanhar leitura pendente dos seus próprios alunos.
     */
    if (notice.studentId) {
      return false;
}
...
const managementNotices = notices.filter((notice) => {
  ...
  if (isTeacher) {
    /*
     * Regra:
     * professor só vê avisos da gestão direcionados a professores.
     * Avisos da gestão direcionados a alunos não aparecem para professor.
     */
...
function getNoticeTargetLabel(notice: (typeof notices)[number]): string {
  ...
  return 'Público não informado';
}
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="bg-[#111111] border border-[#ffffff10] rounded-2xl p-6 md:p-8">
        ...
        <p className="mt-2 text-[#a1a1a1]">
          Bem-vindo ao painel administrativo. Aqui está o resumo das atividades pendentes.
</p> </div>
...
<div className="divide-y divide-[#ffffff10]">
      ...
      <div className="bg-[#111111] border border-[#ffffff10] rounded-2xl p-6 md:p-8">
        ...
        {questionsWithoutAnswer.length === 0 ? (
          <p className="text-[#a1a1a1]">
            Nenhuma dúvida aguardando resposta.
          </p>
): (
<div className="divide-y divide-[#ffffff10]">
        ...
        <div key={notice.id} className="py-4">
          ...
          <p className="text-[#a1a1a1] mt-2">
            ...
            <span className="text-[#D4A373]">
              Por: {notice.author?.name || 'Gestão'}
          ...
          <div key={message.id} className="py-4">
            ...
            {lastReply ? (
              <div className="mt-3 p-3 rounded-lg bg-[#0a0a0a] border border-[#ffffff10]">
                <p className="text-[#a1a1a1] text-sm">
                  Última resposta de {lastReply.answeredBy?.name || 'Professor'}:
</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {students.map((student) => (
                ...
                >
                  ...
                  <p className="text-[#a1a1a1] text-sm">
                    Professor: {student.user?.name || 'Não vinculado'}
</p> </div>
))} </div>
)} </div>
      </div>
