...
export default function TreinosPage() {
  ...
  return <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-6 text-[#f5f5f5]">
    <div className="max-w-6xl mx-auto space-y-6">
      <div><p className="text-xs uppercase tracking-[0.3em] text-[#22D3EE] mb-2">Controle do professor</p><h1 className="text-2xl font-bold text-[#22D3EE]">Treinos dos alunos</h1><p className="text-sm text-[#a1a1a1] mt-2">Visualize todos os treinos e edite manualmente os que ainda não foram concluídos.</p></div>
      <label className="block text-sm text-[#b5b5b5]">Selecione o aluno
        <select value={selectedStudentId} onChange={(e) => setSelectedStudentId(e.target.value)} className="mt-2 w-full bg-[#111] border border-[#ffffff10] rounded-xl px-4 py-3 outline-none focus:border-[#22D3EE] text-white">
      ...
      {loading ? <div className="bg-[#111] rounded-2xl p-6 text-[#a1a1a1]">Carregando...</div> : !selectedStudent ? <div className="bg-[#111] border border-[#ffffff10] rounded-2xl p-6 text-[#a1a1a1]">Selecione um aluno para visualizar os treinos.</div> : <section key={selectedStudent.id} className="bg-[#111] border border-[#ffffff10] rounded-2xl p-4 md:p-5">
        ...
        {selectedStudent.workoutPlans.length === 0 ? <p className="text-sm text-[#777]">Nenhum treino gerado.</p> : <div className="grid gap-3">{selectedStudent.workoutPlans.map((plan) => {
          ...
          return <div key={plan.id} className="bg-[#181818] border border-[#ffffff10] rounded-xl p-4 flex flex-col md:flex-row md:items-center gap-4">
            <div className="flex-1"><div className="flex flex-wrap gap-2 items-center"><h3 className="font-semibold">{plan.name}</h3><span className="text-[11px] border border-[#22D3EE]/30 text-[#22D3EE] rounded-full px-2 py-1">{statusLabel(status)}</span></div><p className="text-xs text-[#888] mt-2">{date ? new Date(date).toLocaleDateString("pt-BR") : "Sem data"} · {plan.exercises.length} exercício(s)</p></div>
            <button onClick={() => openEditor(selectedStudent, plan, completed)} className="px-4 py-2 rounded-lg bg-[#22D3EE] text-black font-semibold">{completed ? "Visualizar treino" : "Editar manualmente"}</button>
          </div>})}</div>}
      </section>}
    </div>
    ...
    {editing && draft && <div className="fixed inset-0 z-50 bg-black/80 overflow-y-auto p-3 md:p-6"><div className="max-w-5xl mx-auto bg-[#111] border border-[#ffffff15] rounded-2xl p-4 md:p-6 space-y-5">
      <div className="flex justify-between gap-4"><div><p className="text-xs text-[#22D3EE]">{editing.student.name}</p><h2 className="text-xl font-bold">{editing.readOnly ? "Visualizar treino" : "Editar treino manualmente"}</h2></div><button onClick={() => { setEditing(null); setDraft(null); }} className="text-[#aaa]">Fechar</button></div>
