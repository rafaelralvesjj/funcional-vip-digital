...
export default function TreinosPage() {
  ...
  const [editing, setEditing] = useState<{ student: Student; plan: Plan } | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((student) => `${student.name} ${student.email || ""}`.toLowerCase().includes(term));
  }, [students, search]);
  ...
  function openEditor(student: Student, plan: Plan) {
    setEditing({ student, plan });
  ...
  return <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-6 text-[#f5f5f5]">
    <div className="max-w-6xl mx-auto space-y-6">
      <div><p className="text-xs uppercase tracking-[0.3em] text-[#D4A373] mb-2">Controle do professor</p><h1 className="text-2xl font-bold text-[#D4A373]">Treinos dos alunos</h1><p className="text-sm text-[#a1a1a1] mt-2">Visualize todos os treinos e edite manualmente os que ainda não foram concluídos.</p></div>
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar aluno..." className="w-full bg-[#111] border border-[#ffffff10] rounded-xl px-4 py-3 outline-none focus:border-[#D4A373]" />
      {error && <div className="border border-red-500/20 bg-red-500/10 text-red-300 rounded-xl p-3 text-sm">{error}</div>}
      {loading ? <div className="bg-[#111] rounded-2xl p-6 text-[#a1a1a1]">Carregando...</div> : filtered.map((student) => <section key={student.id} className="bg-[#111] border border-[#ffffff10] rounded-2xl p-4 md:p-5">
        <div className="mb-4"><h2 className="font-bold text-lg">{student.name}</h2><p className="text-xs text-[#777]">{student.email || "Sem e-mail"}</p></div>
        {student.workoutPlans.length === 0 ? <p className="text-sm text-[#777]">Nenhum treino gerado.</p> : <div className="grid gap-3">{student.workoutPlans.map((plan) => {
          ...
          return <div key={plan.id} className="bg-[#181818] border border-[#ffffff10] rounded-xl p-4 flex flex-col md:flex-row md:items-center gap-4">
            <div className="flex-1"><div className="flex flex-wrap gap-2 items-center"><h3 className="font-semibold">{plan.name}</h3><span className="text-[11px] border border-[#D4A373]/30 text-[#D4A373] rounded-full px-2 py-1">{statusLabel(status)}</span></div><p className="text-xs text-[#888] mt-2">{date ? new Date(date).toLocaleDateString("pt-BR") : "Sem data"} · {plan.exercises.length} exercício(s)</p></div>
            <button disabled={completed} onClick={() => openEditor(student, plan)} className="px-4 py-2 rounded-lg bg-[#D4A373] text-black font-semibold disabled:opacity-40 disabled:cursor-not-allowed">{completed ? "Somente visualizar" : "Editar manualmente"}</button>
          </div>})}</div>}
      </section>)}
    </div>
    ...
    {editing && draft && <div className="fixed inset-0 z-50 bg-black/80 overflow-y-auto p-3 md:p-6"><div className="max-w-5xl mx-auto bg-[#111] border border-[#ffffff15] rounded-2xl p-4 md:p-6 space-y-5">
      <div className="flex justify-between gap-4"><div><p className="text-xs text-[#D4A373]">{editing.student.name}</p><h2 className="text-xl font-bold">Editar treino manualmente</h2></div><button onClick={() => { setEditing(null); setDraft(null); }} className="text-[#aaa]">Fechar</button></div>
      ...
      ].map(([field,label]) => <label key={field} className="text-xs text-[#aaa]">{label}<input value={draft[field] ?? ""} onChange={(e) => setDraft({ ...draft, [field]: e.target.value })} className="mt-1 w-full bg-[#1a1a1a] border border-[#ffffff10] rounded-lg px-3 py-2 text-white" /></label>)}</div>
      {[["description","Descrição"],["studentSummary","Resumo para o aluno"],["safetyNote","Orientação de segurança"],["notes","Observações do professor"]].map(([field,label]) => <label key={field} className="block text-xs text-[#aaa]">{label}<textarea value={draft[field] ?? ""} onChange={(e) => setDraft({ ...draft, [field]: e.target.value })} rows={2} className="mt-1 w-full bg-[#1a1a1a] border border-[#ffffff10] rounded-lg px-3 py-2 text-white" /></label>)}
      <div><h3 className="font-semibold mb-3">Exercícios</h3><div className="space-y-3">{draft.exercises.map((exercise: any, index: number) => <div key={`${exercise.id || exercise.libraryExerciseId}-${index}`} className="bg-[#181818] border border-[#ffffff10] rounded-xl p-3"><div className="flex justify-between gap-2 mb-3"><strong>{index + 1}. {exercise.name}</strong><div className="flex gap-2"><button onClick={() => move(index,-1)}>↑</button><button onClick={() => move(index,1)}>↓</button><button onClick={() => setDraft({ ...draft, exercises: draft.exercises.filter((_: any, i: number) => i !== index) })} className="text-red-400">Excluir</button></div></div><div className="grid grid-cols-2 md:grid-cols-5 gap-2">{[["series","Séries"],["reps","Repetições"],["weight","Carga"],["restTime","Descanso"],["notes","Observação"]].map(([field,label]) => <label key={field} className="text-[11px] text-[#888]">{label}<input value={exercise[field] ?? ""} onChange={(e) => updateExercise(index,field,e.target.value)} className="mt-1 w-full bg-[#222] border border-[#ffffff10] rounded px-2 py-2 text-white" /></label>)}</div></div>)}</div></div>
      <div className="bg-[#181818] rounded-xl p-3"><p className="text-sm font-semibold mb-2">Adicionar exercício da biblioteca</p><input value={librarySearch} onChange={(e) => setLibrarySearch(e.target.value)} placeholder="Pesquisar exercício..." className="w-full bg-[#222] border border-[#ffffff10] rounded-lg px-3 py-2 mb-2" /><div className="max-h-48 overflow-y-auto grid md:grid-cols-2 gap-2">{filteredLibrary.map((item) => <button key={item.id} onClick={() => setDraft({ ...draft, exercises: [...draft.exercises, { libraryExerciseId: item.id, name: item.name, series: 3, reps: "10", weight: "", restTime: "60s", notes: "" }] })} className="text-left bg-[#222] hover:border-[#D4A373] border border-[#ffffff10] rounded-lg p-2 text-sm">+ {item.name}<span className="block text-[10px] text-[#777]">{item.muscleGroup || ""}</span></button>)}</div></div>
      <div className="flex justify-end gap-3"><button onClick={() => { setEditing(null); setDraft(null); }} className="px-4 py-3 rounded-lg bg-[#222]">Cancelar</button><button disabled={saving || draft.exercises.length === 0} onClick={save} className="px-5 py-3 rounded-lg bg-[#D4A373] text-black font-bold disabled:opacity-50">{saving ? "Salvando..." : "Salvar alterações"}</button></div>
    </div></div>}
  </div>;
}
