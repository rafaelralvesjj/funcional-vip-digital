...
export async function GET(
  ...
  try {
    ...
    const completedWorkouts = workouts.filter((workout) => ["CONCLUIDO", "CONCLUIDO_PARCIALMENTE"].includes(workout.status));
    const overdueWorkouts = workouts.filter(
      (workout) => !["CONCLUIDO", "CONCLUIDO_PARCIALMENTE"].includes(workout.status) && workout.date < now
    );
    ...
    const currentWeekCompleted = currentWeekWorkouts.filter((workout) => ["CONCLUIDO", "CONCLUIDO_PARCIALMENTE"].includes(workout.status)).length;
    const partiallyCompletedWorkouts = workouts.filter((workout) => workout.status === "CONCLUIDO_PARCIALMENTE");
    ...
    const summaryText = [
      ...
      `Treinos concluídos (totais + parciais): ${completedWorkouts.length}`,
      `Treinos concluídos parcialmente: ${partiallyCompletedWorkouts.length}`,
      ...
      "Treino concluído parcialmente conta como treino realizado para adesão. Porém, o exercício não realizado não pode receber progressão automática: deve ser mantido, regredido, explicado melhor ou substituído conforme o motivo informado pelo aluno.",
      "",
