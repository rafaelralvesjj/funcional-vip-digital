...
export async function GET(request: NextRequest) {
  try {
    ...
    const normalizedManagers = managers.map(normalizeUser);

    return NextResponse.json(
      {
        managers: normalizedManagers,
        gestores: normalizedManagers,
        items: normalizedManagers,
        data: normalizedManagers,
      },
      {
        headers: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ok: true,
      manager: normalizeUser(created),
    });
  } catch (error: any) {
    console.error("POST /api/managers error:", error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: "ID do gestor é obrigatório." }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ok: true,
      manager: normalizeUser(updated),
    });
  } catch (error: any) {
    console.error("PUT /api/managers error:", error);
...
export async function DELETE(request: NextRequest) {
  try {
     ...
     * Isso evita apagar histórico, avisos e ações vinculadas ao gestor.
     */
    const updated = await prisma.user.update({
      where: {
        id,
    ...
    return NextResponse.json({
      ok: true,
      manager: updated,
      message: "Gestor desativado com segurança.",
    });
  } catch (error: any) {
    console.error("DELETE /api/managers error:", error);
