...
export async function GET(request: NextRequest) {
  try {
    ...
    /*
     * Modo leve usado na abertura da tela Montar treino.
     *
     * A consulta completa desta rota inclui aluno, professor, contrato,
     * treino relacionado e cálculo de impacto comercial. Isso é necessário
     * na central de cuidado, mas é carga desnecessária para apenas decidir se
     * existe alerta ou pausa que bloqueia a prescrição.
     */
    if (viewParam === "workout-builder-summary" && studentIdParam) {
      ...
      return NextResponse.json({
        ...
        summary: {
          ...
          hasTrainingPause: events.some(
            (event) =>
              String(event.eventType).toUpperCase() ===
              "PAUSA_POR_CUIDADO"
          ),
          hasReturnRequest: events.some(
            (event) =>
              String(event.eventType).toUpperCase() ===
                "PAUSA_POR_CUIDADO" &&
              (String(event.status).toUpperCase() === "EM_REVISAO" ||
                String(event.resolutionNotes || "")
                  .toLowerCase()
                  .includes("aptidão de retomada"))
          ),
        },
      });
    }
